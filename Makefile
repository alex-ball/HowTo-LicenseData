...
pdf: tmp $(BIB).bib dcchowto-apa.csl
	...
	pandoc -s -S --latex-engine=lualatex --biblio $(BIB).bib --csl dcchowto-apa.csl -N -V fontsize=11pt -V papersize=a4paper -V lang=british -V geometry:hmargin=3cm -V geometry:vmargin=2.5cm -V mainfont=Charis\ SIL -V monofont=DejaVu\ Sans\ Mono -V header-includes="\usepackage{footmisc}\usepackage[svgnames]{xcolor}\colorlet{dccblue}{Blue}\colorlet{dccmaroon}{Crimson}\colorlet{dccpeach}{white}\let\nonzeroparskip\relax\let\fullcite\textbf\usepackage{tikz}\usetikzlibrary{positioning}" $(NAME)-tmp.md -o $(NAME)-preview.pdf
